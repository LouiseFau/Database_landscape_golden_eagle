...
for (ind in seq_along(files)) {
      ...
      # Step 2.1 : load and read the data in R ---------------------------------------
      eagle <- readRDS(list_file[ind])
      
      # Step 2.2 : Transform categorical variables into continuous variables ---------------------------------------
      # Filter the dataset to keep the individuals with more than 30 locations
      eagle_subset <- eagle %>%
            group_by(individual.local.identifier) %>%
            filter(n() >= 30) %>%
      ...
      # Step 2.5 : give a weight per individuals --------------------------------------
      ...
      # Step 2.6 : calculate the mean, var, min, max for each variables ---------------
      summary <- rbindlist(lapply(ind_list1, function(indi_data) {
            indi_summary <- indi_data %>%
                  select_if(is.numeric) %>%
                  summarise(across(everything(), .fns = list(
      ...
      # Step 2.7 : merge the dataset with the Corine Land Cover dataset ---------------
      tot_eagle <- cbind(summary, vegetation)
      
      # Setp 2.8 : suppress the variables pre-selected
